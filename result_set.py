...
class ResultSet:
    ...
    def append(self, results):
        ...
        for (
            result_name,
            (result_data, result_description, result_physical, result_a_exponent),
        ) in results.items():
    ...
    def get_metadata(self, comm):
        ...
        if len(self) > 0:
            ...
            my_metadata = list(
                zip(names, sizes, units, dtype, descr, physical, a_exponent)
            )
        else:
...
def get_metadata_from_chunk_file(filename, halo_prop_list, reg):
    ...
    with h5py.File(filename, 'r') as file:
        ...
        # We need the metadata to be sorted for comparing with the metadata from other chunks
        for name in sorted(datasets):
            if len(file[name].shape) == 1:
                size = ()
            else:
