...
def build_matrix(flattened_matrix):
    ...
    matrix = sw.cosmo_array(
        np.ones((number_of_matricies, ndim, ndim)),
        units=flattened_matrix.units,
        cosmo_factor=flattened_matrix.cosmo_factor,
        comoving=flattened_matrix.comoving,
    )
    ...
    matrix[matrix_index, row_idx[diagonal], col_idx[diagonal]] = flattened_matrix[
        :, :ndim
    ]
    matrix[matrix_index, row_idx[off_diagonal], col_idx[off_diagonal]] = (
        flattened_matrix[:, ndim:]
    )
    matrix[matrix_index, col_idx[off_diagonal], row_idx[off_diagonal]] = (
        flattened_matrix[:, ndim:]
    )
...
if __name__ == "__main__":
    ...
    for ndim in [2, 3]:
        ...
        random_flattened_matrix = sw.cosmo_array(
            np.random.random((number_test_matricies, entries)),
            units=sw.units.unyt.Mpc,
            comoving=False,
            cosmo_factor=sw.cosmo_factor(sw.a**1, scale_factor=1),
        )
        ...
        assert (
            np.diagonal(reconstructed_matrix, axis1=1, axis2=2)
            == random_flattened_matrix[:, :ndim]
        ).all()
        ...
        # in the function we are testing. NOTE: if I do not use .value, the hstack fails...
