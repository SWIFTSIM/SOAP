\paragraph{$^{$FOOTNOTE_NUMBER$}$The Doppler B parameter}\label{footnote:$FOOTNOTE_NUMBER$} is computed as in \href{https://ui.adsabs.harvard.edu/abs/2017MNRAS.467..985R}{Roncarelli et al. (2018)}:

\begin{equation}
    b = \frac{\sigma{}_T}{c} \sum_i n_{e,i} v_{r,{\rm{}obs},i} \frac{m_i}{\rho{}_i A_{\rm{}obs}},
\end{equation}

where $\sigma_T$ is the Thomson cross section, $c$ the speed of light, $n_{e,i}$ the electron number density 
for gas particle $i$, with $V_i=m_i/\rho_i$ the corresponding SPH particle volume. The relative 
