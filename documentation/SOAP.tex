...
on which particles to included (by radius, in projection...). For all halo types the halo
centre and the boundedness of the particles are determined by the input subhalo catalogue.
...
membership status. They are stored within the group \verb+InclusiveSphere/XXXkpc+.
...
\paragraph{}Note that quantities are given in the base units of the simulation snapshot. 
We therefore recommend that swiftsimio is used to load the catalogues as it has unit handling built in.
The attributes of each SOAP dataset contains all the relevant meta-data to convert between physical
