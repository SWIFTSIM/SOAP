...
The softened $v_{\rm{}max}$ value is calculated using the same method, except that the particle
radius has a floor equal to the softening length. An alternative way to calculate $v_{\rm{}max}$
...
Note that the softened vmax is $\textit{not}$ the $v_{\rm{}max}$ computed using a softened potential.
